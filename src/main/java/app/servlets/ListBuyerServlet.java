                ...
                for (String productID : productsID) {
                    ...
                    for(Product product : Model.getInstance().getList()){
                        if(String.valueOf(product.getId()).equals(productID.split(" ")[0])){
                            Model.getInstance().getCurrentUser().basketList.add(product);
                            Model.getInstance().addToBasket(Model.getInstance().getCurrentUser(), product.getId()+" ");
                            System.out.println(Model.getInstance().getCurrentUser().basketList);

                        }
                    }

                }
//                System.out.println(Model.getInstance().getCurrentUser().getBasket().toString());
//                Model.getInstance().addToBasket(Model.getInstance().getCurrentUser(), basket);
//                req.setAttribute("basket", Model.getInstance().getCurrentUser().getBasket().getList());
            }
        }catch (NullPointerException e){
            req.setAttribute("nullData", "");
            doGet(req, resp);
        }
//        System.out.println(Basket.getInstance().getBasket().get(0).getPrice());
